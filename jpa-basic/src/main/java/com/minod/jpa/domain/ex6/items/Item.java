package com.minod.jpa.domain.ex6.items;

import com.minod.jpa.domain.ex5.Category;
...
@DiscriminatorColumn(name="DTYPE") // DTYPE(지정안해줘도 기본값 DTYPE임) 이라는 컬럼을 추가로 만들고, subtype 테이블이름을 기본값으로 넣어준다.(구분목적으로 넣기위해 만듬) 자식 entity로 추가할 때 자동으로 DTYPE 에 넣어줌.
// 근데이거 이미 테이블 생긴 상태에서 추가하니 에러난다, 아래에 별도로 dtype필드를 추가하면 중복으로잡혀서 추가하면 안됨., DB에 컬럼을 별도로 추가하던지 해야함.
// 예제 사진을 보면 싱글테이블로 되어있음.
public class Item extends BaseEntity { // 상속으로 속성만 추가
    @Id
    @GeneratedValue
    private Long id;
    ...
    @ManyToMany(mappedBy="items") // Category 엔티티 바라봄
